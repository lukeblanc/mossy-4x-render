...
class Broker:
    ...
    def place_order(self, side: str, size: float = 1.0):
        ...
        if side not in ("BUY", "SELL"):
            print(f"[BROKER] Ignoring unknown side: {side}", flush=True)
            return {"status":"IGNORED"}

        # DEMO: simulate only
        if self.mode != "live" or not (self.key and self.account):
            print(f"[BROKER] ({self.mode}) SIMULATED {side} order size={size}", flush=True)
            return {"status":"SIMULATED"}

        # LIVE: send real market order
        units = str(int(size if side == "BUY" else -size))
        payload = {
            "order": {
                "type": "MARKET",
                "instrument": "EUR_USD",   # TODO: make configurable
                "units": units
            }
        }
        try:
            with self._client() as c:
                ...
                if r.status_code in (200, 201):
                    print(f"[BROKER] LIVE {side} sent size={size} resp=OK", flush=True)
